    ...
    for emoji_encontrado in lista_emojis_encontrados:
        ...
        if ruta_imagen:
            ...
            imagen = imagen.resize(
                (40, 40), Image.LANCZOS
            )  # Ajusta el tamaño del emoji a 30x30 píxeles con filtro LANCZOS
            ...
            texto_con_emojis = texto_con_emojis.replace(emoji_encontrado, "")
            emojis += 1

    return texto_con_emojis, emojis
...
def analizador_lexicográfico(entrada):
    ...
    palabras_espanol = VerificadorPalabras(
        "emoji_parser-main/diccionario.txt"
    )
    total_palabras = palabras_espanol.contar_palabras_espanol(entrada)
    print(f"Se encontraron {total_palabras} palabras y {total_emojis} emojis.")
